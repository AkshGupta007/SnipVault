...
// //         <p className="text-gray-400 text-sm mb-6">Start saving your snippets</p>
...
// import { Eye, EyeOff, Lock, Mail, ArrowRight, Users } from "lucide-react";
...
const Register = () => {
  ...
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [success, setSuccess] = useState(false); // ← true after registration
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post("/auth/register", form);
      setSuccess(true); // ← show success screen, don't navigate
    } catch (err: any) {
      setError(err.response?.data?.message || "Registration failed");
    } finally {
      setLoading(false);
      ...
      <div className="relative z-20 flex min-h-screen items-center justify-center px-4">
        ...
        >
          ...
          {success ? (
            ...
            >
              <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-full border border-emerald-400/30 bg-emerald-500/10">
              ...
              <p className="text-indigo-300 font-medium mb-4">{form.email}</p>
              <p className="text-gray-500 text-xs leading-relaxed">
                Click the link in the email to activate your account. <br />
                The link expires in 24 hours.
              </p>
              <div className="mt-8 border-t border-white/10 pt-5 w-full text-center">
            ...
            <>
              ...
              <div className="mb-8 text-center">
                ...
                <p className="mt-3 text-gray-400">Start saving your snippets</p>
              </div>
              ...
              <div className="mt-8 border-t border-white/10 pt-5 text-center">
                <p className="text-xs text-gray-500">
                  © {new Date().getFullYear()} SnipHub • Secure Code Snippet
                  Manager
                </p>
              </div>
            </>
